...
\begin{frame}[t,fragile]{\subsecname}{\subsubsecname}
\begin{itemize} 
\item<1-> 除了高阶的readGDAL函数之外，rgdal包还提供了\emphText{GDAL.open}函数，这个函数不会和
readGDAL那样直接读取数据到R的工作区，而是读取到底层GDAL对象中，这个对象存储了外部文件句柄，可以对文件进行操作和信息读取；当真正需要读取数据的时候再通过\emphText{asSGDF\_GROD}函数将GDAL对象转换成sp对象
\item<1-> 由于GDAL.open函数存储了文件句柄，因此在用完后要调用\emphText{GDAL.close}函数释放掉
\end{itemize}
\end{frame}
