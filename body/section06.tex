...
\begin{itemize}
\item<1-> R属于开源生态圈，因此外部GIS文件和sp对象的相互转换也主要\emphText{采用开源GIS的解决方案}
\item<2-> 除了商业的GIS软件产品(如 ArcGIS、SuperMap等)之外，世界上还有一些\emphText{非营利性组织负责维护和管理大量的开源GIS项目}，其中比较著名的是\href{https://www.osgeo.org/}{\uline{地理空间开源基金会}}(Open Source Geospatial Foundation)
\end{itemize}
...
\only<2>{
  \begin{table} \centering \small
    ...
    \begin{tabular}{|>{\centering\arraybackslash} m{0.2\columnwidth} |>{\centering\arraybackslash} m{0.3\columnwidth} |>{\centering\arraybackslash} m{0.35\columnwidth}|}
      ...
      二次开发包 & ArcEngine & GDAL、GeoTools、proj.4\\\hline
