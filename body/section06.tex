...
\begin{frame}[t,fragile]{\subsecname}{\subsubsecname}
\begin{itemize}
\item<1-> R属于开源生态圈，因此外部GIS对象和sp对象的转换也主要采用开源GIS的解决方案，一方面可以导入外部GIS数据到R中，另一方面也可以将sp对象导出到外部GIS格式文件
\item<2-> 除了商业的GIS软件(如ESRI ArcGIS)之外，还有一些\emphText{非营利性组织负责维护和管理大量的开源GIS项目}，其中比较著名的是\href{https://www.osgeo.org/}{\uline{地理空间开源基金会}}(Open Source Geospatial Foundation)
\end{itemize}
...
\begin{table} \centering \scriptsize
    ...
    \begin{tabular}{>{\centering\arraybackslash} m{0.3\columnwidth} >{\centering\arraybackslash} m{0.1\columnwidth} >{\centering\arraybackslash} m{0.5\columnwidth}}
      \toprule
      \rowcolor{LightCyan}
      \multicolumn{1}{c}{\textbf{GIS技术线}} & \multicolumn{1}{c}{\textbf{商业解决方案}} &
\multicolumn{1}{c}{\textbf{开源解决方案}} \\\hline
      桌面GIS & ArcMap & QGIS、GRASS GIS \\
      webGIS & element\_text & x,y轴标签\\
      空间数据库 & element\_line & axis刻度线 \\
      地图服务 & element\_text & axis刻度标签\\
      二次开发包 & element\_rect & legend背景\\
      元数据管理 & element\_rect & legend标示背景\\
      \bottomrule
    \end{tabular}
