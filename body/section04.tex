...
\begin{frame}[t]{\subsecname}{}
\begin{itemize}
\item R的base包中包括标准绘图程序包\emphText{graphics}，除了包括前面介绍的所有统计元素之外，
还包含大量预置的统计图形
\item 绘图程序包通过\emphText{高级绘图函数}在R中绘制图形
\item graphics程序包中预置的统计图形库已经涵盖了绝大多数常见的统计图形；
但除此之外，\emphText{contrib包中也包括大量绘图程序包来进一步完善统计图形库}
\end{itemize}
\end{frame}
...
\item<3-> 使用grid绘图系统前需要先加载\emphText{grid程序包}，该程序包由奥克兰大学的Paul Murrell开发维护 
\end{itemize}
...
不像graphis程序包还集成了高级绘图函数用于绘制常用的统计图形，因此要直接用grid程序包绘制统计图形比较复杂
\end{itemize}
\end{block}}
...
\only<4>{
\begin{block}{lattice包和ggplot2包是grid绘图系统的两种实现}
因为直接用grid包绘制统计图形比较复杂，所以有专门基于grid包开发的绘图程序包，提供
高级绘图函数来简化统计图形的绘制过程，比较著名的是\emphText{lattice}和\emphText{ggplot2}
\end{block}}
...
\caption{grid绘图系统与graphics绘图系统的关系}
\end{figure}}
...
\begin{frame}[t,fragile]{\subsecname}{}
\begin{itemize}
\item<1-> lattice程序包是基于grid的编写的一套统计图形系统
\item<2-> grid绘图系统与graphics程序包截然不同，是另一套全新的绘图模式，
其\emphText{设计初衷是为了克服基础统计图形系统中元素不能动态修改的弱点}
\item<3-> 使用grid绘图系统前需要先加载\emphText{grid程序包}，该程序包由PaulMurrell开发维护
\item<3-> 虽然在相同的绘图设备上，同一会话里用两种绘图系统是被允许的，
但是\emphText{grid图形通常不和graphics图形混用}
\end{itemize}

\begin{overlayarea}{\textwidth}{\textheight}
\only<2>{
\begin{block}{grid绘图系统和基础统计图形库的区别}
\begin{itemize} 
\item[\HandRight] 用\emphText{视口(viewports)}实现绘图设备的分割，
绘图对象(grob)可以在不同的视口中进行共享，比graphics中的处理方式更加灵活
\item[\HandRight] 绘图对象可以被修改或者从一个图形中移除，\emphText{而不需要重新绘制所有的图形}，
但是在graphics中则必须重绘 
\end{itemize}
\end{block}}
