...
视化的方法}，其所有绘图要素被保存在一个\emphText{trellis对象}中，并且在\emphText{嵌板(panel)}中绘制，
...
\begin{onlyenv}<2>
\begin{figure}\centering
    \includegraphics[width=0.6\columnwidth]{panel-example1.png}
    \caption{在高级绘图函数xyplot中自定义panel和strip的示例}
\end{figure}
\end{onlyenv} 
\end{frame}
...
\begin{frame}[t,fragile]{\subsecname}{图形参数和主题}
\begin{itemize}
\item 在lattice中，所有的trellis对象都包含一个主题(theme)，theme中拥有完整的图形要素设置
\item 当前theme的参数可以通过\emphText{trellis.par.get()}函数获取，通过
\emphText{trellis.par.set()}函数修改
\item 与par()函数不同，\emphText{theme中的参数对于所有高级绘图函数都是一致的}，
