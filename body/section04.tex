...
\item<1-> 使用grid绘图系统前需要先加载\emphText{grid程序包}，
...
\begin{frame}[t,fragile]{\subsecname}{}
\begin{itemize}
\item lattice程序包是基于grid包的编写的一套统计图形库，由
\href{https://www.isid.ac.in/~deepayan/}{\uline{Deepayan Sarkar}}开发维护
\item lattice设计理念来自S-PLUS中的Trellis图形，是一种\emphText{多元数据可
视化的方法}，其所有绘图要素被保存在一个\emphText{trellis对象}中，并在\emphText{嵌板(panel)}中绘制
\item lattice包从R 3.0版本开始已纳入base包，\emphText{不需要再单独安装}，但是要用
\emphText{library(lattice)}语句进行加载
\end{itemize}
...
\begin{frame}[t]{\subsecname}{高级绘图函数}
\begin{itemize}
\item lattice提供了大量高级绘图函数用于绘制常用的统计图形
\end{itemize}
\begin{figure}\centering
    \includegraphics[width=0.65\columnwidth]{lattice-all.png}
    \caption{lattice中的高级绘图函数}
\end{figure}
\end{frame} 
...
\begin{frame}[t]{\subsecname}{低级绘图函数}
\begin{itemize}
\item<1-> lattice通过高级绘图函数中的panel参数对图形添加附加要素，\emphText{panel参数本身是一个匿名函数}，
类似于低级绘图函数的作用
\item<2-> lattice也可以通过\emphText{update()}函数修改既有trellis对象中的绘图要素，从而达到修改图形的目的
\end{itemize}
\end{frame} 

\begin{frame}[t]{\subsecname}{主题}
\begin{itemize}
\item 所有的trellis对象都有一个主题(theme)，用于定义颜色、尺寸、宽度、大小等图形要素
\item 主题有点类似于par()函数，但是par()函数无法统一graphics包中的所有高级绘图函数，
而在\emphText{主题中的参数设置对于lattice包中所有高级绘图函数都是一致的}，这样就可以通过指定主题来达到
图形风格的统一
\end{itemize}
\end{frame}
