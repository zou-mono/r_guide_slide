...
\subsection{数据操作}
\begin{frame}{\subsecname}{数据类型(mode)}
  \begin{itemize}
...
\begin{frame}[t]{\subsecname}{对象和数据结构}
  \begin{itemize}
  \item 在R语言中，\emphText{几乎任何东西都是对象}；使用对象的好处是可以复用，提升自动化程度
  \item R语言中为了提高数据的使用效率，预定义了一些数据结构，\emphText{R语言会根据
        这些数据结构来创建对象}
  \item 这些数据结构支撑了R强大的统计分析能力
  \end{itemize}
